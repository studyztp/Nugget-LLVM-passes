...
1. **Inserts `nugget_init` call**: At the end of `nugget_roi_begin_`, it inserts a call to `nugget_init(total_bb_count)` to initialize the runtime with the total number of basic blocks.

2. **Instruments all basic blocks**: Every basic block that has `!bb.id` metadata (from IRBBLabelPass) gets a `nugget_bb_hook(inst_count, bb_id, threshold)` call inserted before its terminator instruction.

3. **Skips nugget functions**: Functions like `nugget_init`, `nugget_roi_begin_`, `nugget_roi_end_`, and `nugget_bb_hook` are not instrumented to avoid infinite recursion.
...
└── test3_no_label/
    └── CMakeLists.txt       # Negative test: no prior IRBBLabelPass
```
...
- `nugget_init` is called with the correct total BB count
- All labeled basic blocks have `nugget_bb_hook` calls
- Hook calls receive correct inst_count, bb_id, and threshold arguments
...
- `nugget_init` call appears in `nugget_roi_begin_`
- `nugget_bb_hook` calls appear in the disassembly
...
2. **Checks `nugget_init` call**: Verifies it exists in `nugget_roi_begin_` with correct argument
3. **Checks `nugget_bb_hook` calls**: Verifies each labeled BB has a hook call with matching IDs
...
5. **Verifies `nugget_init` call** appears in `nugget_roi_begin_`
...
define void @nugget_roi_begin_() {
entry:
  call void @nugget_init(i64 42)  ; 42 = total BB count
  ret void
}
...
entry:
  ; ... computation ...
  call void @nugget_bb_hook(i64 5, i64 0, i64 1000)  ; inst_count=5, bb_id=0, threshold=1000
  ret i32 %result, !bb.id !0
}
```
...
The `nugget_bb_hook` function signature is:
```c
void nugget_bb_hook(uint64_t inst_count, uint64_t bb_id, uint64_t threshold);
```

Where:
- `inst_count`: Number of LLVM IR instructions in this basic block
- `bb_id`: Global basic block ID from the CSV (unique across the module)
- `threshold`: Interval length for phase analysis (in IR instructions executed)
